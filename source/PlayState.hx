...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			detailsText = "Story Mode: " + MainMenuState.weekName2;
		}
		else
		{
		...
		switch (curStage)
		{
			...
			case "norfairStage":
				...
				var bg:BGSprite = new BGSprite('stages/norfairStage/norfair_caveback', posX, posY, 0.9, 0.9);
				bg.scale.set(scaleThing, scaleThing);
				bg.updateHitbox();
				bg.antialiasing = false;
				add(bg);

				var polesBack:BGSprite = new BGSprite('stages/norfairStage/norfair_polesback', posX, posY, 0.9, 0.9);
				polesBack.scale.set(scaleThing, scaleThing);
				polesBack.updateHitbox();
				polesBack.antialiasing = false;
				add(polesBack);

				var polesFront:BGSprite = new BGSprite('stages/norfairStage/norfair_polesfront', posX, posY, 0.9, 0.9);
				polesFront.scale.set(scaleThing, scaleThing);
				polesFront.updateHitbox();
				polesFront.antialiasing = false;
				add(polesFront);

				var platform:BGSprite = new BGSprite('stages/norfairStage/norfair_platform', posX, posY, 0.9, 0.9);
				platform.scale.set(scaleThing, scaleThing);
			...
			case "torizoLair":
				...
				GameOverSubstate.characterName = 'bf-helm';

				var bg:BGSprite = new BGSprite('stages/torizoStage/bgback', -357, -186, 0.9, 0.9);
				add(bg);

				var cliffs:BGSprite = new BGSprite('stages/torizoStage/bgbackcliffs', -123, 80, 0.9, 0.9);
				cliffs.setGraphicSize(Std.int(cliffs.width * 0.7), Std.int(cliffs.width * 0.7));
				cliffs.updateHitbox();
				add(cliffs);

				var poles:BGSprite = new BGSprite('stages/torizoStage/bgbackpoles', -270, -67, 0.9, 0.9);
				poles.setGraphicSize(Std.int(poles.width * 0.8), Std.int(poles.width * 0.8));
				poles.updateHitbox();
				add(poles);

				var ground:BGSprite = new BGSprite('stages/torizoStage/bgfront', -117, -74, 0.9, 0.9);
				ground.setGraphicSize(Std.int(ground.width * 0.8), Std.int(ground.width * 0.8));
				ground.updateHitbox();
				add(ground);
		}
